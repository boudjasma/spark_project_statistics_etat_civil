...
object project extends App {
  ...
  //Analyse with age
  val Married_by_age = All.filter(All("MaritalStatus") like "Married").groupBy("AgeGroup").count().withColumnRenamed("count","NumberOfMarried")
  val Divorced_by_age = All.filter(All("MaritalStatus") like "Divorced").groupBy("AgeGroup").count().withColumnRenamed("count","NumberOfDivorced")
  val Single_by_age = All.filter(All("MaritalStatus") like "Single").groupBy("AgeGroup").count().withColumnRenamed("count","NumberOfSingle")

  val max_married_by_age = Married_by_age.sort(desc("NumberOfMarried")).limit(5)
  max_married_by_age.show()

  val max_Divorced_by_age = Divorced_by_age.sort(desc("NumberOfDivorced")).limit(5)
  max_Divorced_by_age.show()

  val max_Single_by_age = Single_by_age.sort(desc("NumberOfSingle")).limit(5)
